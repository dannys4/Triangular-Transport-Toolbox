...
# Use option whether to make plots or not
make_plots = True
...
tm = transport_map(
    # Specify the monotone parts of the map component function
    monotone=monotone,
    # Specify the nonmonotone parts of the map component function
    nonmonotone=nonmonotone,
    # A N-by-D matrix of training samples (N = ensemble size, D = variable space dimension)
    X=X,
    # What types of polynomials did we specify? The option 'Hermite functions' here are re-scaled probabilist's Hermites, to avoid numerical overflow for higher-order terms
    polynomial_type="hermite function",
    # Are we ensuring monotonicity through 'integrated rectifier' or 'separable monotonicity'?
    monotonicity="integrated rectifier",
    # Standardize the training ensemble X? Should always be True
...
if 'dict_coeffs_order='+str(maxorder)+'.p' not in os.listdir(root_directory):
    ...
    # Save the dictionary
    pickle.dump(dict_coeffs, open(
        'dict_coeffs_order='+str(maxorder)+'.p', 'wb'))
    ...
    dict_coeffs = pickle.load(
        open('dict_coeffs_order='+str(maxorder)+'.p', 'rb'))
...
if make_plots:
    ...
    plt.scatter(X[:, 0], X[:, 1], s=1, color='xkcd:orangish red',
                label='target samples', zorder=10)
    plt.scatter(Z_gen[:, 0], Z_gen[:, 1], s=1,
                color='xkcd:grass green', label='reference samples', zorder=5)
...
if make_plots:
    ...
    plt.scatter(X_gen[:, 0], X_gen[:, 1], s=1,
                color='xkcd:orangish red', label='target samples', zorder=10)
    plt.scatter(Z[:, 0], Z[:, 1], s=1, color='xkcd:grass green',
                label='reference samples', zorder=5)
    plt.xlabel('dimension 1 ($x_{1}$ or $z_{1}$)')
    plt.ylabel('dimension 2 ($x_{2}$ or $z_{2}$)')
...
if make_plots:
    ...
    gs = matplotlib.gridspec.GridSpec(
        ...
        wspace=0.)
    plt.subplot(gs[0, 0])
    plt.scatter(X[:, 0], X[:, 1], s=1, color='xkcd:silver',
                label='target samples', zorder=5)
    plt.scatter(X_cond[:, 0], X_cond[:, 1], s=1, color='xkcd:orangish red',
                label='conditional samples', zorder=10, alpha=0.01)
    ...
    plt.legend()
    plt.subplot(gs[0, 1])
    plt.hist(X_cond[:, 1], orientation='horizontal',
             bins=30, color='xkcd:orangish red')
    plt.ylim(-3, 3)
