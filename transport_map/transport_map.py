...
class transport_map():
    ...
    def __init__(self,
        ...
        if self.ST_scale_mode not in ['dynamic', 'static']:
            raise ValueError(
                "'ST_scale_mode' must be either 'dynamic' or 'static'.")
        ...
        if self.adaptation_map_type == 'separable':
            ...
            while iterate:
                ...
                if iteration >= maxorder_nonmon:
                    print(
                        "WARNING: Map adaptation stopped at maximum number of iterations.")
                    iterate = False
    ...
    def reset(self, X):
        ...
        if len(X.shape) != 2:
            raise Exception(
                'X should be a two-dimensional array of shape (N,D), N = number of samples, D = number of dimensions. Current shape of X is '+str(X.shape))
    ...
    def precalculate(self):
        ...
        for k in range(self.D):
            ...
            self.Psi_mon    .append(
                copy.copy(self.fun_mon[k](copy.copy(self.X), self)))
        ...
        # If the entry is a string, it denotes a special term
            ...
            if STtype.lower() == "let":  # The special term is a left edge term
                ...
                if mode == 'standard':
                    ...
                    # Construct the string
                    string = "((__x__[...,"+i+"] - __mu__)*(1-scipy.special.erf((__x__[...,"+i + \
                        "] - __mu__)/(np.sqrt(2)*__scale__))) - __scale__*np.sqrt(2/np.pi)*np.exp(-((__x__[...," + \
                        i+"] - __mu__)/(np.sqrt(2)*__scale__))**2))/2"
                    ...
                    if int(i) == k:
                        ...
                        string = "(1 - scipy.special.erf((__x__[...," + \
                            i+"] - __mu__)/(np.sqrt(2)*__scale__)))/2"
            ...
            # -----------------------------------------------------------------
                ...
                if mode == 'standard':
                    ...
                    # Construct the string
                    string = "np.exp(-((__x__[...,"+i + \
                        "] - __mu__)/__scale__)**2/2)/(__scale__*np.sqrt(2*np.pi))"
            ...
            # -----------------------------------------------------------------
                ...
                if mode == 'standard':
                    ...
                    if int(i) == k:
                        ...
                        string = "1/(np.sqrt(2*np.pi)*__scale__)*np.exp(-(__x__[...," + \
                            i+"] - __mu__)**2/(2*__scale__**2))"
    ...
    def function_constructor_alternative(self, k=None):
        ...
        for k in Ks:
                    ...
                    if variable_choice in modifier_log:
                        ...
                        for (key, value) in modifier_log[variable_choice].items():
                            if variable_choice == "cached_variables":
                                if key in my_dict:
                                    value = merge_cached_variables(
                                        key, my_dict[key], value, self.polyfunc_str)
                                my_dict[key] = value

                            # Have we logged this one already?
                            if key not in my_dict:
                                # No, we haven't. Add it.
                                my_dict[key] = copy.copy(
                                    value).replace("__x__", "x")
            ...
            if 'cross-terms' in list(self.special_terms[k+self.skip_dimensions].keys()):
                ...
                RBF_terms_grid = copy.deepcopy(
                    RBF_terms_dict[RBF_terms_dims[0]])
            ...
            if not partial_construction:
                ...
                self.coeffs_mon     .append(
                    np.ones(len(terms))*self.coeffs_init)
        ...
        for k in range(self.D):
            ...
            for entry in self.nonmonotone[k]:
                ...
                if type(entry) == str:
                    ...
                    # Mark it in memory
                    self.special_terms[k +
                                       self.skip_dimensions][index]['counter'] += 1
                ...
                if type(entry) == str:
                    ...
                    # Does this monotone term have cross-terms?
                        ...
                        if 'cross-terms' not in list(self.special_terms[k+self.skip_dimensions].keys()):
                            ...
                            self.special_terms[k +
                                               self.skip_dimensions]['cross-terms'] = {}
                        ...
                        # Mark it in memory
                        self.special_terms[k +
                                           self.skip_dimensions]['cross-terms'][index]['counter'] += 1
            ...
            for d in keylist:
                ...
                if dictionary[d]['counter'] == 1:
                    ...
                    if self.ST_scale_mode == 'dynamic':  # Dynamic scales
                        ...
                        dictionary[d]['scales'] = np.asarray(
                ...
                # ---------------------------------------------------------
                    ...
                    quantiles = np.arange(
                        1, dictionary[d]['counter']+1, 1)/(dictionary[d]['counter']+1)
                    ...
                    if self.ST_scale_mode == 'dynamic':
                        ...
                        for i in range(dictionary[d]['counter']):
                            ...
                            if i == 0:
                                ...
                                scales[i] = \
                                    (dictionary[d]['centers'][1] - dictionary[d]
                                     ['centers'][0])*self.ST_scale_factor

                            # Right edge-case: base is half distance to previous basis
                            elif i == dictionary[d]['counter']-1:

                                scales[i] = \
                                    (dictionary[d]['centers'][i] - dictionary[d]
                                     ['centers'][i-1])*self.ST_scale_factor

                            # Otherwise: base is average distance to neighbours
                            else:

                                scales[i] = \
                                    (dictionary[d]['centers'][i+1] - dictionary[d]
                                     ['centers'][i-1])/2*self.ST_scale_factor
                        ...
                        # Copy the scales into the array
                        dictionary[d]['scales'] = copy.copy(
                            scales) + self.ST_scale_factor
    ...
    def optimize(self, K=None):
            ...
            if self.verbose:
                ...
                for i in range(len(task_supervisor)):
                    if task_supervisor[i] == 1:     # Successful task
                        ...
                        # Unfinished task (should not occur)
                        string += ' '
                string += '|'
                print(string)
        ...
        for k in range(self.D):
            ...
            # Create the function
            funstring = "fun_nonmon_"+str(k)
            exec(self.fun_nonmon_strings[k].replace(
                "fun", funstring), globals())
            print(funstring)
            exec("self.fun_nonmon.append(copy.deepcopy("+funstring+"))")
            ...
            if self.monotonicity == "separable monotonicity":
                ...
                # Create the function
                funstring = "der_fun_mon_"+str(k)
                exec(self.der_fun_mon_strings[k].replace(
                    "fun", funstring), globals())
                exec("self.der_fun_mon.append(copy.deepcopy("+funstring+"))")
    ...
    def worker_task_monotone(self, k, task_supervisor):
        ...
        if self.regularization is None:
            ...
            A_sqrt = self.Psi_mon[k] - \
                np.linalg.multi_dot((Q, Q.T, self.Psi_mon[k]))
            ...
            def fun_mon_objective(coeffs_mon, A, k, all_outputs=True):
                ...
                objective = np.dot(coeffs_mon[np.newaxis, :], Ax)[
                    0, 0]/2 - np.sum(logdS)/N + np.inner(coeffs_mon, b)
    ...
    def worker_task(self, k, task_supervisor):
        ...
        if self.workers > 1:
            ...
            for i in range(self.D):
                ...
                exec(self.fun_mon_strings[i].replace(
                    "fun", funstring), globals())
                exec("self.fun_mon.append(copy.deepcopy("+funstring+"))")

                # Create the function
                funstring = "fun_nonmon_"+str(i)
                exec(self.fun_nonmon_strings[i].replace(
                    "fun", funstring), globals())
                exec("self.fun_nonmon.append(copy.deepcopy("+funstring+"))")
        ...
        if self.regularization is not None:
            ...
            if type(self.regularization) == str:
                ...
                if self.regularization.lower() == 'l1':
                    ...
                    raise ValueError(
                        "regularization_type must be either 'l1' or 'l2'.")
    ...
    def vectorized_root_search_bisection(self, X, Zk, k, max_iterations=100,
        ...
        failure = np.isnan(X[:, self.skip_dimensions + k])
        ...
        while len(shiftindices) > 0:
            ...
            X[shiftindices[failure_pos], self.skip_dimensions +
                k] = copy.copy(bsct_pts[shiftindices[failure_pos], 0])
            ...
            X[shiftindices[failure_neg], self.skip_dimensions +
                k] = copy.copy(bsct_pts[shiftindices[failure_neg], 1])
            ...
            # Where the product has different signs, zero is in-between
            failure = np.where(
                np.prod(bsct_out[shiftindices, :], axis=1) > 0)[0]
            shiftindices = shiftindices[failure]
    ...
    def Quadrature(self, f, a, b, order=100, args=None, Ws=None,
        ...
        if use_scipy:
            ...
            integrand = f
            if args is not None:
                def integrand(t): return f(t, *args)
            if not (np.isscalar(a) and np.isscalar(b)):
                ba_diff = b-a
                a_copy = copy.copy(a)
                if args is not None:
                    def integrand(t): return f(t*ba_diff + a_copy, *args)
                else:
                    def integrand(t): return f(t*ba_diff + a_copy)
                a, b = 0., 1.
            if full_output:
                raise ValueError(
                    "Cannot return full output with scipy quadrature")
    ...
    def projectedNewton(self, x0, fun, args=None, method='trueHessian', rtol_Jdef=1E-6, rtol_gdef=1E-6, itmaxdef=30, epsilon=0.01):
        ...
        def Armijo(xk, gk, pk, Jk, Ik, fun, args=None, itmax=15, sigma=1E-4, beta=2):
            ...
            while Jk < Jxk_alpha_lin and it < itmax:
                ...
                Jxk_alpha = fun(xk_alpha[:, 0], A=args[0],
                                k=args[1], all_outputs=False)
    ...
    def adaptation_cross_terms(self, increment=1E-6, chronicle=False):
        ...
        for k in range(self.D):
            ...
            multi_index_matrix = np.zeros(
                tuple([self.adaptation_max_order+1]*(k+1+self.skip_dimensions)), dtype=int)
            ...
            coeffs = np.asarray(
                list(copy.copy(self.coeffs_nonmon[k])) + list(copy.copy(self.coeffs_mon[k])))
            ...
            while repeat:
                ...
                nonzero_cells = np.asarray(
                    np.where(self.multi_index_matrix > 0)).T
                ...
                proposed_cells = np.asarray(
                    np.where(self.multi_index_matrix <= -(k+1+self.skip_dimensions))).T
                ...
                for idx, cell in enumerate(proposed_cells):
                    ...
                    coeffs_new = np.ones(
                        len(nonmonotone) + len(monotone))*self.coeffs_init + increment
                ...
                coeffs_new = np.ones(len(nonmonotone) +
                                     len(monotone))*self.coeffs_init
                coeffs_new[orig] = copy.copy(coeffs)
    ...
    class rectifier:
        ...
        RectifierStrings = ['SOFTPLUS', 'SQUARED',
                            'EXPONENTIAL', 'EXPNEG', 'EXPLINEARUNIT']
        Rectifier = Enum("Rectifier", RectifierStrings)
        RectifierDict = {str(rec).split(
            '.')[1].lower(): rec for rec in Rectifier}
